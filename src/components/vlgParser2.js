...
const moduleParser = A.contextual(function*() {
  ...
  const id = yield ws(identifier).errorMap((error, index) =>
    throwLintError(
      `"module" keyword should be followed by a valid identifier: ${error}`,
      index
    )
  );
  ...
  return {
    parseState: A.many1(
      ws(moduleParser).errorMap((error, index) => {
        ...
        return throwLintError(`Invalid module definition: ${error}`, index);
      })
    ).run(code),
